...
function PixelBasedPIOverTime_func(filename,pathname)
...
frfig= figure; imshow(fr); title('fr');
% Save this figure
figname = strcat(filename(1:end-5),'frfig.fig'); 
saveas(frfig,figname); 
jpgname = strcat(filename(1:end-5),'frfig.jpg'); 
saveas(frfig,jpgname);
...
ADfig = figure; imshow(meanADcirc); title('mean AD circ')
% Save this figure
figname = strcat(filename(1:end-5),'ADfig.fig'); 
saveas(ADfig,figname); 
jpgname = strcat(filename(1:end-5),'ADfig.jpg'); 
saveas(ADfig,jpgname);
...
matFilename = strcat(filename(7:end-5),'_PI.mat');
save(matFilename, 'PIfrIndx', 'PIfrPx','filename'); 
% Save the PI over Time plot as a jpg: 
jpgFilename = strcat(filename(7:end-5),'_PI.jpg'); 
saveas(gcf,jpgFilename); 
figFilename = strcat(filename(7:end-5),'_PI.fig'); 
...
disp('Finished processing'); disp(filename); 
disp('in'); disp(pathname); 